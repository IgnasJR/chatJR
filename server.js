        ...
        connection.query(
          ...
          (err, result) => {
            if (err) {
              ...
              console.log(
                "Added conversation between users:",
                userId,
                "and",
                otherUserId
              );
...
app.post("/messages", verifyToken, (req, res) => {
  ...
  const query =
    "INSERT INTO Messages (sender_id, conversation_id, message_content, created_at) VALUES (?, ?, ?, ?)";
  connection.query(
    query,
    [req.userId, conversationId, messageContent, new Date()],
    (err, result) => {
      if (err) {
        console.error("Error executing MySQL query:", err);
